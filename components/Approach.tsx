...
function ModalityItem({ modality }: { modality: typeof CORE_MODALITIES[0] }) {
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(248,246,242,0.12)' }}>
      <button
        onClick={() => setOpen(o => !o)}
      ...
      >
        <div className="pb-8 grid md:grid-cols-3 gap-6 md:gap-10">
          <div>
            <p style={{ fontSize: '0.65rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(248,246,242,0.35)', marginBottom: '0.6rem' }}>What it is</p>
            <p className="font-garamond text-lg leading-relaxed" style={{ color: 'rgba(248,246,242,0.7)' }}>{modality.what}</p>
          </div>
          <div>
            <p style={{ fontSize: '0.65rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(248,246,242,0.35)', marginBottom: '0.6rem' }}>The benefit</p>
            <p className="font-garamond text-lg leading-relaxed" style={{ color: 'rgba(248,246,242,0.7)' }}>{modality.benefit}</p>
          </div>
          <div>
            <p style={{ fontSize: '0.65rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(248,246,242,0.35)', marginBottom: '0.6rem' }}>Best for</p>
            <p className="font-garamond text-lg leading-relaxed" style={{ color: 'rgba(248,246,242,0.7)' }}>{modality.who}</p>
          </div>
        </div>
      </div>
    </div>
...
export default function Approach() {
  ...
  return (
    <section id="approach" className="relative overflow-hidden" style={{ background: '#0D0D0D' }}>
      <div className="flex flex-col md:flex-row md:min-h-screen">
        ...
        <div className="flex flex-col justify-center w-full md:w-[55%] px-6 md:pl-16 lg:pl-24 md:pr-12 py-16 md:py-24">
          ...
          <h2 className="font-cormorant font-light text-[clamp(2.2rem,6vw,4.5rem)] leading-[1.05] tracking-tight mb-10 md:mb-12" style={{ color: '#F8F6F2' }}>
            Restore Self-Trust
          </h2>
          <div className="max-w-2xl">
            <div className="font-garamond text-xl md:text-2xl leading-relaxed space-y-6 text-left" style={{ color: 'rgba(248,246,242,0.7)' }}>
              <p>Many people who have been scapegoated, gaslit, or emotionally overridden have been taught to mistrust themselves. They may know something is wrong but struggle to explain it — because the body and nervous system often hold the truth long before the mind can put it into words.</p>
              <p>The tools I use are not abstract concepts. They are practices I turned to while rebuilding after chronic family stress and the long-term bodily cost of suppressed truth. I select what is most appropriate for each person depending on their nervous system, history, and stage of recovery.</p>
              <p>Rebuilding often begins the moment someone is finally believed — and helped to believe themselves.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 md:px-16 lg:px-24 pb-24 md:pb-32">
        <p style={{ fontSize: '0.65rem', letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(248,246,242,0.35)', marginBottom: '2rem' }}>The Tools Behind the Process</p>
        <h3 className="font-cormorant font-light text-[clamp(1.8rem,4vw,3rem)] leading-tight tracking-tight mb-10" style={{ color: '#F8F6F2' }}>What supports the work</h3>
        ...
        <div className="mt-10">
          ...
          >
            ...
            <span style={{ display: 'inline-block', transition: 'transform 0.3s ease', transform: showExtended ? 'rotate(45deg)' : 'rotate(0deg)', color: 'rgba(248,246,242,0.3)', fontSize: '1rem', lineHeight: 1 }}>+</span>
          </button>
          <div style={{ overflow: 'hidden', maxHeight: showExtended ? '2000px' : '0', transition: 'max-height 0.5s ease' }}>
            <div className="mt-8" style={{ borderBottom: '1px solid rgba(248,246,242,0.12)' }}>
              <p className="font-cormorant font-light text-xl mb-6" style={{ color: 'rgba(248,246,242,0.4)', fontStyle: 'italic' }}>
